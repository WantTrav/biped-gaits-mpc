...
#include <fstream>
#include <time.h>
#include <Eigen/StdVector>   ///c++98
#include <vector> 
#include <boost/math/special_functions/round.hpp>    //c++98
...
const double M_PI = 3.141592653;             ////////////c++98 
...
public:
	...
	///c++98
	void CoM_foot_trajection_generation_local(int i, const Eigen::Matrix<double,18,1>& estimated_state, Eigen::Vector3d _Rfoot_location_feedback, Eigen::Vector3d _Lfoot_location_feedback,double lamda,bool _ISwalking);
...
///    offline calculated matrices	
	...
	std::vector <Eigen::Matrix3d> _xkzk_constraints;	
	//std::vector <Eigen::Matrix<double,3,3>, Eigen::aligned_allocator<Eigen::Matrix<double,3,3>>> _xkzk_constraints;	
...
//	void solve_AAA_inv(Eigen::Matrix<double, 4, 1> t_plana); 
	void solve_AAA_inv(const Eigen::Matrix<double, 4, 1>& t_plan); 
